...
def compute_g0(outcar_name, plot_min_wavelength, plot_max_wavelength, plot, cavity_length, semiconductor_length):
    ...
    dataim = extract_dielectric_function(
        lines, "frequency dependent IMAGINARY DIELECTRIC FUNCTION (independent particle, no local field effects) current-current"
    )
    datareal = extract_dielectric_function(
        lines, "frequency dependent      REAL DIELECTRIC FUNCTION (independent particle, no local field effects) current-current"
    )
    ...
    prefactor = 4.2*1e-6 # phonon mean free path in GaAs at room temperature (https://doi.org/10.1038/srep02963)
...
def compute_g0_2nd_approx(outcar_name, plot_min_wavelength, plot_max_wavelength, plot, cavity_length, semiconductor_length, gradient_value):
    assert plot_max_wavelength > plot_min_wavelength, "Invalid wavelength range for plotting"
    assert cavity_length >= semiconductor_length, "Semiconductor must fit inside cavity"
    ...
    g_0_array = filtered_omega * (1 - filtered_refractive_index - semiconductor_length*1e6*gradient_value) / denominator * prefactor
